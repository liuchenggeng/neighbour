<template>
  <el-dialog
    ...
    <el-form-item label="业委会状态(0未成立,1发起中,2已成立,3过期)" prop="status">
      <el-input v-model="dataForm.status" placeholder="业委会状态(0未成立,1发起中,2已成立,3过期)"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          status: [
            { required: true, message: '业委会状态(0未成立,1发起中,2已成立,3过期)不能为空', trigger: 'blur' }
          ],
          showStatus: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/house/committee/info/${this.dataForm.id}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.createName = data.committee.createName
                this.dataForm.createBy = data.committee.createBy
                this.dataForm.createTime = data.committee.createTime
                this.dataForm.updateName = data.committee.updateName
                this.dataForm.updateBy = data.committee.updateBy
                this.dataForm.updateTime = data.committee.updateTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/house/committee/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
